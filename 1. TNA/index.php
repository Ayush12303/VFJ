...
if (isset($_POST['submit'])) {
    ...
    $sql = "INSERT INTO $table 
            VALUES ($id, '$name', $personalNo, '$designation', '$dateOfBirth', '$section', '$dateOfJoiningService', '$qualification', '$natureOfPresentJob', '$course1', '$course2', '$course3', '$trainingArea1', '$trainingArea2', '$trainingArea3', '$knowledgeImprove', '$technologyDevelopments', '$selfDevelopments', '$futureAssignments', '$recommendation1', '$recommendation2', '$recommendation3', '$recommendation4', '$recommendation5', '$recommendation6')";
...
<body>
    <div class="container">
		...
		<br><br>
		<h4>(12) Recommendation of DO/GO/concerned Section indicating the training course / field to be imparted</h4>
            <div>
                <label for="recommendation1">i.</label>
                <textarea id="recommendation1" name="Recommendation1" rows="1" placeholder=""   oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength="254" ></textarea>
            </div>
            <div>
                <label for="recommendation2">ii.</label>
                <textarea id="recommendation2" name="Recommendation2" rows="1" placeholder=""   oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength="254" ></textarea>
            </div>
            <div>
                <label for="recommendation3">iii.</label>
                <textarea id="recommendation3" name="Recommendation3" rows="1" placeholder=""   oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength="254" ></textarea>
            </div>
			
			
			
            <h4>(13) Recommendation of trg. officer in view of the training programme available</h4>
            <div>
                <label for="recommendation4">i.</label>
                <textarea id="recommendation4" name="Recommendation4" rows="1" placeholder=""   oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength="254" ></textarea>
            </div>
            <div>
                <label for="recommendation5">ii.</label>
                <textarea id="recommendation5" name="Recommendation5" rows="1" placeholder=""   oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength="254" ></textarea>
            </div>
            <div>
                <label for="recommendation6">iii.</label>
                <textarea id="recommendation6" name="Recommendation6" rows="1" placeholder=""   oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength="254" ></textarea>
            </div>
			
			<div class="signatures" style="text-align: right;">
                <h4>(Signature & Post Box of DO/GO/CO)</h4>
            </div>
